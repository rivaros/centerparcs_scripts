...
if [ ! -z "$1" ]; then
      ...
      echo "Usage: install_bucardo <user>"
      exit
fi
...
bucardo set log_conflict_file=/opt/local/var/log/bucardo_conflict.log
bucardo set reason_file=/opt/local/var/log/bucardo.restart.reason.log
bucardo set warning_file=/opt/local/var/log/bucardo.warning.log
...
#Start Bucardo
bucardo start --debugdir=/opt/local/var/log
