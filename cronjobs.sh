...
function check() {
	...
	if [ `uname` == "Darwin" ];then
			...
			launchctl getenv PATH | grep "/opt/local/lib/postgresql91/bin:/opt/local/bin:/opt/local/sbin" > /dev/null 2>&1
			if (( $? ));then 
				if [[ $1 ]];then echo "ERROR: Launchctl PATH problems in Darwin environment";else return 0;fi
			fi
	fi
...
function install() {
	...
	if [ `uname` == "Darwin" ];then
		    #Transfer checker will be running under Apache user
       		    chown _www $BINROOT/transfer_checker.sh
